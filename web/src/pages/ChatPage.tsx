...
/**
 * StatusBar — compact info bar at the bottom of the chat panel.
 * Displays model name, context usage, and token count in a Codex-style layout.
 * Currently uses placeholder data pending real-time session metrics implementation.
 */
function StatusBar() {
  // Placeholder state - to be replaced with real session data later
  const modelInfo = {
    model: "Auto",
  ...
  const formatContext = (used?: number, total?: number) => {
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="tracking-wide">
            {formatContext(modelInfo.tokensUsed, modelInfo.contextWindow)}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="tracking-wide font-mono">
          {formatTokens(modelInfo.tokensUsed)}
        </span>
      </div>
    </div>
  );
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          ...
          <div
            ref={hostRef}
            className="hermes-chat-xterm-host min-h-0 flex-1"
            style={{ padding: "12px" }}
          />

          {/* Status bar - Codex style */}
          <StatusBar />

          {/* Floating copy button - repositioned for better UX */}
          <Button
            ghost
            ...
            className={cn(
              ...
              "rounded-md border border-current/30",
              "bg-black/40 backdrop-blur-md",
              "opacity-60 hover:opacity-100 hover:border-current/70",
              "transition-all duration-150",
              "bottom-16 right-3 px-3 py-1.5 text-xs",
              "sm:bottom-20 sm:right-4 sm:px-3.5 sm:py-2",
          ...
          >
            <span className="inline-flex items-center gap-2">
              ...
              <span className="hidden min-[400px]:inline tracking-wide font-medium">
        ...
        {/* Right sidebar - tools and model info */}
        {!narrow && (
          <div
            id="chat-side-panel"
            role="complementary"
            ...
            className={cn(
              ...
              "border border-current/10 rounded-lg",
              "bg-background-base/50 backdrop-blur-sm",
              "transition-all duration-200",
            )}
          >
